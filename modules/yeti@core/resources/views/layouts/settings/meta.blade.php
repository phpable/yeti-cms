@object($Item, 'id', 'type', 'property', 'content')
...
<tr @if ($hidden) style="display: none" @endif @if(!empty($id)) id="{{ $id }}" @endif @if(!empty($Item->id)) data-uid="{{ $Item->id }}" @endif>
	<td data-name="{{ $prefix }}[{{ $action  }}][{{ !empty($Item->id) ? $Item->id : '__UID__' }}][type]" data-type="list" data-options="name,http-equiv">{{ $Item->type }}</td>
	<td data-name="{{ $prefix }}[{{ $action  }}][{{ !empty($Item->id) ? $Item->id : '__UID__' }}][property]" data-type="text">{{ $Item->property }}</td>
	<td data-name="{{ $prefix }}[{{ $action  }}][{{ !empty($Item->id) ? $Item->id : '__UID__' }}][content]" data-type="text">{{ $Item->content }}</td>
